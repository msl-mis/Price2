            ...
            try
            {
                ...
                for (int i = 0; i < dgvData.Rows.Count - 1; i++)
                {
                    strSQL = $@"insert into pld
                                ...
                                values      ('{rstrID}',
                                             '{dgvData.Rows[i].Cells["pld_name"].Value.ToString()}',
                                             {dgvData.Rows[i].Cells["pld_pricost"].Value.ToString()},
                                             {dgvData.Rows[i].Cells["pld_ll"].Value.ToString()},
                                             {dgvData.Rows[i].Cells["pld_hopecost"].Value.ToString()},
                                             {dgvData.Rows[i].Cells["pld_hopell"].Value.ToString()},
                                             '{dgvData.Rows[i].Cells["pld_date"].Value.ToString()}',
                                             {dgvData.Rows[i].Cells["pld_oldll"].Value.ToString()}) ";
                    clsDB.Execute(strSQL);
                }
                if(iCount>0)
                {
