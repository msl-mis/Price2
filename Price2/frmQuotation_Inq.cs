...
namespace Price2
{
    public partial class frmQuotation_Inq : Form
    {

        public static string rstrQuotation = "";   //傳來的報價單
        public static string rstrCustomer = "";   //傳來的客戶
        public frmQuotation_Inq()
        {
            InitializeComponent();
        }

        private void frmQuotation_Inq_Load(object sender, EventArgs e)
        {
            //要加入很多初始化東西
            try
            {
                ...
                if (txtCustomer.Text.Trim() == "" && txtQuotation.Text.Trim() == "Q")
                {
            ...
            {
                MessageBox.Show(this.Name + "-frmQuotation_Inq_Load" + "\n" + ex.Message, "ERROR!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnInq_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (txtDate_E.Text == "" && txtDate_S.Text == "")
                {
                    strSQL = $@"select distinct pdh_orderid'報價單號',
                                                pdh_customer'客戶',
                                                pdh_orderdate'報價日期'
                                from            pdh
        ...
        private void dgvData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    frmQuotation.rstrQuotation = dgvData.Rows[e.RowIndex].Cells["報價單號"].Value.ToString();
                    this.Close();
                }
            }
            catch (Exception ex)
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (clsGlobal.checkRightFlag("客戶報價單單號查詢刪除") == false)
                {
