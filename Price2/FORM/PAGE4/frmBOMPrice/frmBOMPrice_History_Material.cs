...
namespace Price2
{
    public partial class frmBOMPrice_History_Material : Form
    {
        public static string rstrID = "";  //由frmBOMPrice傳的材料ID
        public frmBOMPrice_History_Material()
        {
            InitializeComponent();
        }

        private void frmBOMPrice_History_Material_Load(object sender, EventArgs e)
        {
            //要加入初始化的東西
            try
            {
            ...
            {
                MessageBox.Show(this.Name + "-frmBOMPrice_History_Material_Load" + "\n" + ex.Message, "ERROR!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void GetInq()
        {
            try
            {
                ...
                strSQL = $@"select distinct prb_length    '品號',
                                            prb_customerid'材料名',
                                            prb_date      '變動日期',
                                            prb_pricost   '材料價',
                                            prb_username  '用戶'
                ...
                strSQL = strSQL + strWhere + "order  by '變動日期' desc ";
        ...
        private void btnPrint_Click(object sender, EventArgs e) //列印
        {
            ...
            try
            {
                ...
                clsGlobal.ExportExcel("材料單價史", dgvData);
        ...
        private void GetDelete()
        {
            ...
            {
                //確認權限
                if (clsGlobal.checkRightFlag("材料單查價史刪除") == false)
                {
                    MessageBox.Show("您沒有材料單查價史刪除權限!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnDeleteAll_Click(object sender, EventArgs e) //全部刪除
        {
            ...
            try
            {
                ...
                //確認權限
                if (clsGlobal.checkRightFlag("材料單查價史全部刪除") == false)
                {
                    MessageBox.Show("您沒有材料單查價史全部刪除權限!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Cursor = Cursors.Default;//滑鼠還原預設
                    return;
                }
                //有報價記錄,不能被刪除
                ...
                if (MessageBox.Show("你確認要刪除所有符合這些條件的資料嗎?", "Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        strSQL = $@"delete prb
                                where  prb_customerid = '{rstrID}'
                                       and Format(prb_date, 'yyyy/MM/dd HH:mm:ss') = '{Convert.ToDateTime(dgvData.Rows[i].Cells["變動日期"].Value).ToString("yyyy/MM/dd HH:mm:ss")}' ";
                        clsDB.Execute(strSQL);
                    }
